...
const Marquee: React.FC<{
  ...
  <div className="relative flex overflow-hidden [mask-image:linear-gradient(to_right,transparent,white_10%,white_90%,transparent)]">
    <div
      className={`flex min-w-full shrink-0 items-center justify-around gap-12 motion-safe:animate-marquee-${direction}`}
    >
      {children}
    </div>
    <div
          ...
          <Marquee direction="left">
            {logos.slice(0, 8).map(({ name, Icon }) => (
              <div key={name} aria-label={name} className="flex-shrink-0">
                <Icon className="h-12 w-auto text-text-muted transition-opacity duration-300 opacity-[0.65] hover:opacity-100 focus-visible:opacity-100" />
              </div>
            ))}
          </Marquee>
